...
export default function Addrestaurant(props) {
    ...
    const handleNext = () => {
        if (activeStep === steps.length - 1) {
            ...
            if (Object.values(vals[1]).length != 7 || Object.values(vals[1]).some(a => a.replace(/\s/g, "") == "")) return setError('რესტორნის აღწერა')
            ...
            if (Object.keys(vals[0]).length == 0) return setError("დამატებითი ინფორმაცია")
            if (!Object.values(vals[0]).some(a => a['24'] == true || (a['დან'] && a['მდე']))) return setError("დამატებითი ინფორმაცია")

            senddata.about.id = String(Math.floor(Math.random() * 100000000))

            setSenddata({ ...senddata })

            firebase.auth().onAuthStateChanged(function (user) {
                firebase.firestore().collection('data').doc(user.email).update({ restaurants: firebase.firestore.FieldValue.arrayUnion(senddata) }).then(() => props.history.push("myrestaurants"))
            });
